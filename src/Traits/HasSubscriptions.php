...
use sfwcybersecurity\subscriptions\Exceptions\DuplicateException;
use sfwcybersecurity\subscriptions\Exceptions\InvalidPlanSubscription;
use sfwcybersecurity\subscriptions\Models\Plan;
use sfwcybersecurity\subscriptions\Models\PlanCombination;
use sfwcybersecurity\subscriptions\Models\PlanSubscription;
use sfwcybersecurity\subscriptions\Services\SubscriptionPeriod;
use Carbon\Carbon;
...
trait HasSubscriptions
{
    ...
    {
        return $this->morphMany(config('sfw.models.plan_subscription'), 'subscriber', 'subscriber_type', 'subscriber_id');
    }
    ...
    public function subscription(?string $subscriptionTag = null)
    {
        ...
        $subscriptionTag = $subscriptionTag ?? config('sfw.main_subscription_tag');
     ...
     *
     * @return \sfwcybersecurity\subscriptions\Models\PlanSubscription|null
     */
    public function subscribedPlans()
    {
        $planIds = $this->subscriptions->reject->isInactive()->pluck('plan_id')->unique();

        return app(config('sfw.models.plan'))->whereIn('id', $planIds)->get();
    }
     ...
     * @param \sfwcybersecurity\subscriptions\Models\Plan|\sfwcybersecurity\subscriptions\Models\PlanCombination $planCombination Plan pricing and invoice data
    ...
    {
        $tag = $tag ?? config('sfw.main_subscription_tag');
