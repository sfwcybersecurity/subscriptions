...
namespace sfwcybersecurity\subscriptions\Models;

use BadMethodCallException;
use sfwcybersecurity\subscriptions\Exceptions\UsageDenied;
use sfwcybersecurity\subscriptions\Services\Period;
use sfwcybersecurity\subscriptions\Traits\BelongsToPlan;
use sfwcybersecurity\subscriptions\Traits\HasFeatures;
use sfwcybersecurity\subscriptions\Traits\HasGracePeriod;
use sfwcybersecurity\subscriptions\Traits\HasGracePeriodUsage;
use sfwcybersecurity\subscriptions\Traits\HasPricing;
use sfwcybersecurity\subscriptions\Traits\HasSubscriptionPeriodUsage;
use sfwcybersecurity\subscriptions\Traits\HasTrialPeriodUsage;
use sfwcybersecurity\subscriptions\Traits\HasSchedules;
use Carbon\Carbon;
use DB;
...
class PlanSubscription extends Model
{
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->setTable(config('sfw.tables.plan_subscriptions'));
    }
    ...
    public function getRules(): array
    {
        return [
            'tag' => [
                ...
                'alpha_dash',
                'max:150',
                Rule::unique(config('sfw.tables.plan_subscriptions'))->where(function ($query) {
            ...
            'plan_id' => 'required|exists:' . config('sfw.tables.plans') . ',id',
    ...
    {
        return $this->hasMany(config('sfw.models.plan_subscription_feature'), 'plan_subscription_id', 'id');
    }
    ...
    public function usage(): \Illuminate\Database\Eloquent\Relations\HasManyThrough
    {
        return $this->hasManyThrough(
            config('sfw.models.plan_subscription_usage'),
            config('sfw.models.plan_subscription_feature'),
    ...
    {
        if (!$ignoreFallback && config('sfw.fallback_plan_tag')) { // Do not cancel if a fallback plan is set
            $plan = Plan::getByTag(config('sfw.fallback_plan_tag'));
            if (!$plan) {
                throw new UnexpectedValueException('Fallback plan ' . config('sfw.fallback_plan_tag') . ' does not exist.');
            }
     ...
     * @param int $uses
     *
     * @return \sfwcybersecurity\subscriptions\Models\PlanSubscriptionUsage|null
     */
