...
namespace sfwcybersecurity\subscriptions\Services;
...
class ScheduleService implements PlanSubscriptionScheduleService
{
    ...
    {
        try {
            $payment = app()->make(config('sfw.services.payment_methods.' . $this->planSubscriptionSchedule->subscription->payment_method));
            $payment->charge();
